...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::SKINXXX)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("SKINXXX")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeSkinxxxAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSkinxxxAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeSkinxxxAmount = strAnonymizeSkinxxxAmount.remove(strAnonymizeSkinxxxAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSkinxxxAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeSkinxxxAmount * COIN) nMaxToAnonymize = nAnonymizeSkinxxxAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeSkinxxxAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeSkinxxxAmount));
        strAnonymizeSkinxxxAmount = strAnonymizeSkinxxxAmount.remove(strAnonymizeSkinxxxAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSkinxxxAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
